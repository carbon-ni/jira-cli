...
The tool started with the idea of making issue search and navigation as straightforward as possible. It now includes all
...
## Installation
Build and install `jira-cli` from source:

```sh
make install
```

You can also use Docker to quickly try out `jira-cli`:
...
1. [Get a Jira API token](https://id.atlassian.com/manage-profile/security/api-tokens) and export it to your shell as
   ...
   variable is always available. Alternatively, you can also use `.netrc` file or `keychain` to set the token.
...
1. Export required environment variables:
   ...
   - Alternatively, you can also use `.netrc` file or `keychain` to set the token.
...
You can use a `--custom` flag to set custom fields while creating the issue.
...
The command uses `less` as a pager by default.
...
1. Clone the repo.
   ```sh
   git clone git@github.com:carbon-ni/jira-cli.git
   ```
