...
func (c *JiraCLIConfigGenerator) configureLocalAuthType() error {
	...
	if c.usrCfg.AuthType == "" {
		qs := &survey.Select{
			...
			Help: `Authentication type coud be: basic (login), bearer (PAT), mtls (client certs) or cookie (browser session)
...
? If you are using browser session cookies, the auth type is 'cookie'`,
